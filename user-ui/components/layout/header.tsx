...
import { Menu, MapPin, Store,  ShoppingCart } from "lucide-react"
...
const Header = () => {
  const {user,isSuccess,isError,isLoading,logout,isLogoutSuccess,isLogoutError,isLoggingOut,refetch} = useUser();
  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-white border-b shadow-sm">
      ...
      <div className="container mx-auto px-4 h-16 flex items-center gap-4 lg:gap-6">
        ...
        {/* Logo */}
        <Link href="/" className="flex items-center gap-1 mr-2 shrink-0">
        ...
        <div className="flex items-center gap-2 md:gap-6 shrink-0">
          ...
          {/* User Dropdown */}
          <UserDropdown logout={logout} isLogoutSuccess={isLogoutSuccess} isLogoutError={isLogoutError} isLoggingOut={isLoggingOut} isLoggedIn={isSuccess}  user={user} isLoading={isLoading} isError={isError}  refetch={refetch} />
          ...
          <Link href="/cart" className="flex items-center gap-1 font-bold text-sm hover:text-blue-600 relative group">
            <div className="relative">
              ...
              <Badge className="absolute -top-1 -right-2 h-4 w-4 p-0 flex items-center justify-center      text-[10px] border-white">
                2
              </Badge>
            </div>
